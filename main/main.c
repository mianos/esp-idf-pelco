...
static int cmd_tilt(void *context, int argc, char **argv)
{
    ...
    if (argc < 2) {
        printf("Usage: tilt <tilt_speed (0-255)>\n");
        return 1;
    }
    int tiltSpeed = atoi(argv[1]);
    if (tiltSpeed < 0 || tiltSpeed > 255) {
        printf("Tilt speed must be between 0 and 255.\n");
        return 1;
    }
    
    uint8_t tiltCommand = PELCO_CMD_TILT_CMD;
    uint8_t panSpeed = 0x00;
    uint8_t tiltSpeedByte = (uint8_t)tiltSpeed;
    
    if (!pelco_bus_command(cmdContext->dev, tiltCommand, panSpeed, tiltSpeedByte)) {
        printf("Failed to send tilt command.\n");
        return 1;
    }
    printf("Tilting turret with speed %d.\n", tiltSpeed);
    return 0;
}
...
static int cmd_rotate(void *context, int argc, char **argv)
{
    cmd_context_t *cmdContext = (cmd_context_t *)context;
    if (argc < 2) {
        printf("Usage: rotate <rotate_speed (0-255)>\n");
        return 1;
    }
    int rotateSpeed = atoi(argv[1]);
    if (rotateSpeed < 0 || rotateSpeed > 255) {
        printf("Rotate speed must be between 0 and 255.\n");
        return 1;
    }
    
    uint8_t rotateCommand = PELCO_CMD_ROTATE_CMD;
    uint8_t panSpeed = (uint8_t)rotateSpeed;
    uint8_t tiltSpeed = 0x00;
    
    if (!pelco_bus_command(cmdContext->dev, rotateCommand, panSpeed, tiltSpeed)) {
        printf("Failed to send rotate command.\n");
        return 1;
    }
    printf("Rotating turret with speed %d.\n", rotateSpeed);
    return 0;
}
